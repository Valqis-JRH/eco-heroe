...
export default function EcoHeroe() {
  ...
  // --- FUNCIÓN INTELIGENTE: PROBAR MODELO ---
  async function probarModelo(modelo: string, base64Data: string) {
    console.log(`Intentando con: ${modelo}...`);
    
    // Seguridad desactivada para evitar bloqueos falsos
    ...
    const prompt = `Analiza la imagen. Responde SOLO JSON: {"nombre": "objeto", "puntos": 10, "esReciclable": true}. Si no: {"nombre": "Nada", "puntos": 0, "esReciclable": false}`;
  ...
  const capturarYAnalizar = async () => {
    ...
    try {
        ...
        const modelos = [
            ...
            "gemini-1.5-flash-8b", 
            "gemini-1.5-pro",
            "gemini-pro" // El último recurso
        ];

        for (const m of modelos) {
            try {
                text = await probarModelo(m, base64Data);
                setModeloUsado(m); // ¡ÉXITO!
                exito = true;
                break; // Salimos del bucle
            } catch (error) {
                console.warn(`Falló ${m}, probando siguiente...`);
            }
        }

        if (!exito) throw new Error("Ningún modelo respondió. Verifica API Key.");
        ...
        setMensaje({ texto: `Error V9: ${error.message}`, tipo: 'error' });
    }
  ...
  if (cargandoDatos) return <div className="min-h-screen bg-green-800 flex flex-col items-center justify-center text-white">Cargando V9...</div>;

  return (
    <div className="min-h-screen bg-green-50 text-gray-800 font-sans flex justify-center items-center p-4">
      <div className="w-full max-w-sm h-[800px] bg-white rounded-[40px] border-8 border-green-800 overflow-hidden relative shadow-2xl flex flex-col">
        
        {/* HEADER V9 (VERDE OSCURO) */}
        <div className="pt-12 px-6 pb-6 bg-green-700 rounded-b-3xl shadow-lg z-10">
          <div className="flex justify-between items-center mb-4">
             <div className="flex items-center gap-2">
                <div className="w-10 h-10 bg-white/20 backdrop-blur rounded-full flex items-center justify-center text-xl">♻️</div>
                <h3 className="font-bold text-white text-lg">EcoHéroe Final</h3>
             </div>
             <div className="bg-green-900 px-3 py-1 rounded-full text-xs text-green-200 font-mono border border-green-500">v9.0</div>
          </div>
        ...
        <div className="flex-1 px-6 pt-8 overflow-y-auto pb-20 bg-green-50">
          <button 
            ...
            className="w-full bg-white p-6 rounded-3xl shadow-xl border border-green-200 flex items-center gap-4 group hover:scale-[1.02] transition-transform mb-8"
          >
             ...
             <div className="text-left">
                ...
                <p className="text-green-600 text-sm">Multi-Modelo IA</p>
             </div>
          </button>
                 ...
                 <div className="relative h-full bg-gray-900 flex flex-col items-center justify-center p-6">
                    ...
                    {analizando ? (
                        <div className="text-center">
                            ...
                            <p className="text-green-400 font-mono animate-pulse">Buscando modelo disponible...</p>
                        </div>
                    ) : (
                        <div className="bg-white w-full p-6 rounded-3xl text-center animate-slide-up">
                            <p className="text-xs text-gray-400 mb-2">Conectado a: {modeloUsado}</p>
                            {puntosGanados > 0 ? (
                                <>
                                    <div className="text-5xl mb-2">♻️</div>
                                    <h2 className="text-2xl font-black text-gray-800">{materialDetectado}</h2>
                                    <div className="bg-green-100 text-green-800 text-xl font-bold py-3 rounded-xl mb-4">+{puntosGanados} Pts</div>
