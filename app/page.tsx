...
export default function EcoHeroe() {
  ...
  const capturarYAnalizar = async () => {
    ...
    try {
        ...
        if (!exito) throw new Error("Fallo total IA.");
      ...
      <div className="w-full max-w-sm h-[800px] bg-white rounded-[40px] border-8 border-green-900 overflow-hidden relative shadow-2xl flex flex-col">
        ...
        <div className="pt-12 px-6 pb-6 bg-green-600 rounded-b-3xl shadow-lg z-10">
          <div className="flex justify-between items-center mb-4">
             ...
             <div className="bg-green-800 px-3 py-1 rounded-full text-xs text-green-200 font-mono border border-green-400">v14.0</div>
          </div>
        ...
        <div className="flex-1 px-6 pt-8 overflow-y-auto pb-20 bg-green-50">
          ...
          >
             ...
             <div className="text-left">
                ...
                <p className="text-green-600 text-sm">IA 2.5 Flash</p>
             </div>
          </button>
                 ...
                 <div className="relative h-full bg-gray-900 flex flex-col items-center justify-center p-6">
                    ...
                    {analizando ? (
                        <div className="text-center">
                           ...
                           <p className="text-green-400 font-mono animate-pulse">Consultando 2.5...</p>
                        </div>
                    ) : (
                        <div className="bg-white w-full p-6 rounded-3xl text-center animate-slide-up">
                            <p className="text-xs text-gray-400 mb-2">Modelo: {modeloUsado}</p>
                            {puntosGanados > 0 ? (
                                <>
                                ...
                                <>
                                    ...
                                    <p className="text-red-400 text-xs mt-2">{mensaje?.texto}</p>
                                    <button onClick={() => setImgSrc(null)} className="w-full bg-gray-200 text-gray-800 font-bold py-3 rounded-xl mt-4">Reintentar</button>
                                </>
                            )}
                        </div>
                    )}
        ...
        {mensaje && mensaje.tipo === 'error' && (
            <div className="absolute top-10 left-1/2 transform -translate-x-1/2 w-11/12 px-6 py-4 bg-red-600 text-white rounded-xl z-50 text-xs">
                {mensaje.texto}
            </div>
        )}
        
        {mensaje && mensaje.tipo === 'exito' && (
          <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 w-11/12 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-bounce z-50 bg-gray-800 text-white">
